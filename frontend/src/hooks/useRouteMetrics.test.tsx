...
describe("useRouteMetrics", () => {
  test("fetches metrics from backend", async () => {
    ...
    const fetchMock = vi.fn(async (url: string) => {
      expect(url).toBe(
        "http://api/route-metrics?pickupLat=1&pickupLon=2&dropoffLat=3&dropoffLon=4&ride_time=2020-01-01T00%3A00%3A00.000Z"
      );
  ...
  test("returns null on failure", async () => {
    ...
    const { result } = renderHook(() => useRouteMetrics());
    expect(
      await result.current(1, 2, 3, 4, "2020-01-01T00:00:00.000Z")
    ).toBeNull();
    expect(
