...
load_dotenv()
from models import User, AdminConfig, Booking, Base
...
#     # Prevent double-booking
#     existing = db.query(Booking).filter(Booking.time == req.datetime).first()
#     if existing:
#         raise HTTPException(status_code=400, detail="Driver already booked at that time.")
...
#     db.add(new_booking)
#     db.commit()
#     db.refresh(new_booking)  # Reload the object with its DB-generated fields
...
def create_booking(
    ...
    new_booking = Booking(
        ...
        pickup_location=req.pickup.address,
        dropoff_location=req.dropoff.address,
        time=req.datetime,
        status="pending",
