...
def upgrade() -> None:
    ...
    bookingstatus = sa.Enum(
        'PENDING', 'DRIVER_CONFIRMED', 'DECLINED', 'ON_THE_WAY',
        'ARRIVED_PICKUP', 'IN_PROGRESS', 'ARRIVED_DROPOFF', 'COMPLETED', 'CANCELLED',
        name='bookingstatus'
    )
    notificationtype = sa.Enum(
        'NEW_BOOKING', 'CONFIRMATION', 'LEAVE_NOW', 'ON_THE_WAY',
        'ARRIVED_PICKUP', 'STARTED', 'ARRIVED_DROPOFF', 'COMPLETED', name='notificationtype'
    )
    ...
    if not inspector.has_table('bookings_v2'):
        op.create_table(
            ...
            sa.Column('customer_id', sa.dialects.postgresql.UUID(as_uuid=True), sa.ForeignKey('users_v2.id'), nullable=False),
    ...
    if not inspector.has_table('trips'):
        op.create_table(
            'trips',
            sa.Column('id', sa.dialects.postgresql.UUID(as_uuid=True), primary_key=True),
            sa.Column('booking_id', sa.dialects.postgresql.UUID(as_uuid=True), sa.ForeignKey('bookings_v2.id'), nullable=False),
    ...
    if not inspector.has_table('notifications'):
        op.create_table(
            'notifications',
            sa.Column('id', sa.dialects.postgresql.UUID(as_uuid=True), primary_key=True),
            sa.Column('booking_id', sa.dialects.postgresql.UUID(as_uuid=True), sa.ForeignKey('bookings_v2.id'), nullable=True),
...
def downgrade() -> None:
    ...
    sa.Enum(name='notificationrole').drop(op.get_bind(), checkfirst=False)
